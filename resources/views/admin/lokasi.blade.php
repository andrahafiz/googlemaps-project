    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h4 class="text-themecolor">Data Rumah Sakit</h4>
            </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <h4 class="card-title">Table Rumah Sakit</h4>
                            </div>
                            <div class="col-2 d-flex justify-content-end align-items-center">
                                <a href="{{ route('admin.lokasi.create') }}"><button type="button"
                                        class="btn btn-info d-none d-lg-block m-l-15 "><i class="fa fa-plus-circle"></i>
                                        Tambah Data</button></a>
                            </div>
                        </div>
                        <div class="table-responsive m-t-40">
                            <table id="example23" class="table product-overview" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        <th>Nomor</th>
                                        <th>Nama</th>
                                ...
                                <tbody>
                                    @foreach ($dtLokasi as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->alamat_rumahsakit }}</td>
                                            {{-- <td>20</td> --}}
                                            <td>
                                                <a href="{{ route('admin.lokasi.show', $item->id_rumahsakit) }}"
                                                    class="text-inverse p-r-10" title="Detail" data-toggle="tooltip"><i
                                                        class="ti-eye"></i></a>
                                                <a href="{{ route('admin.lokasi.edit', $item->id_rumahsakit) }}"
                                                    class="text-inverse p-r-10" data-toggle="tooltip" title="Edit"><i
                                                        class="ti-marker-alt"></i></a>
                                                <a type="submit" role="button" class="text-inverse border-0"
                                                    onclick="event.preventDefault();
                                                    document.getElementById('logout-form{{ $item->id_rumahsakit }}').submit();"
                                                    title="Delete" data-toggle="tooltip"><i class="ti-trash"></i></a>
                                                <form method="POST" {{-- class="btn btn-danger btn-sm p-2 sweet" --}}
                                                    id="logout-form{{ $item->id_rumahsakit }}"
                                                    action="{{ route('admin.lokasi.destroy', $item->id_rumahsakit) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
                                            </td>
